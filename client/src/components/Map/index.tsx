...
const Minimap: React.FC<IMapProps> = ({
  ...
  const [mapImgSrc, setMapImgSrc] = React.useState<string | null>(null);
  ...
  const handleMapMouseMove = (
    ...
    if (event.buttons === 1 && isSpacePressedRef.current) {
      ...
      if (
        ref.current &&
        (Math.abs(newCoordinates.y) > ref.current.offsetHeight ||
          Math.abs(newCoordinates.x) > ref.current.offsetWidth / 1.5)
      ) {
        return;
      }
  ...
  React.useEffect(() => {
    try {
      ...
      image.src = REACT_APP_API_URL + info.image.split("\\").join("/");

      image.onload = () => {
        setMapImgSrc(image.src);
      };
  ...
  return (
    <div className={styles.wrapper}>
      ...
      >
        ...
        >
          {children}
          <img src={mapImgSrc} alt={info.name + " Image"} />
        </div>
      </div>
    </div>
  );
